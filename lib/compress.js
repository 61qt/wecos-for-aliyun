...
const compressEndFix = '.compress.gif';
function compress(config, filePath, toPath, cb) {
	...
	tool.uploadFile(
		...
		filePath,
		toPath + compressEndFix,
		function (err, cosFilepath) {
			...
			var req;
			var picURL = `${cosFilepath}?imageView2/q/70`;
			// console.log('compress', picURL);
