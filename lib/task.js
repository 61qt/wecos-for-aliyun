...
var CWD = process.cwd();
var SUFFIX = ['wxml', 'less', 'wxss', 'jsx'];
var REG_PROTOCOL = /^([a-zA-Z]{2,}:)\/\//;
var REG_IMAGE_TAG = /(<[Ii]mage( [^>]*)? src *= *['|"])(.*?)(['|"])/gi;
var REG_COVER_IMAGE_TAG = /(<cover-image( [^>]*)? src *= *['|"])(.*?)(['|"])/g;
...
var onlineLinks = function(currentPath, resourcePath, url) {
	...
	// console.log('onlineLinks url', url, 'match', match);
	...
	// 找出资源链接，并替换
	var replaceHandler = function(s, m1, m2, oldUrl, m4) {
		var findResourcePath =
			oldUrl.substr(0, 1) === '/' ? pathLib.join(APP, oldUrl) : pathLib.resolve(pathLib.dirname(currentPath), oldUrl);
	...
	content = content.replace(REG_IMAGE_TAG, replaceHandler);
	content = content.replace(REG_IMAGE_STYLE, replaceHandler);
	content = content.replace(REG_COVER_IMAGE_TAG, replaceHandler);
	// 如果文件内容有改变，写入文件
...
var replaceLinks = function(currentPath) {
	...
	content = content.replace(REG_COVER_IMAGE_TAG, _replaceM3);
	...
	fs.lstat(resourcePath, function(err, stat) {
		if (stat.size > 0) {
			uploadFile(resourcePath, function(err, url) {
				// console.log('resourcePath 1', resourcePath, 'err', err, 'url', url);
				if (err) {
					callback(err);
				} else {
					scanFiles(function(currentPath) {
						// console.log('currentPath', currentPath, 'resourcePath', resourcePath， 'url', url);
